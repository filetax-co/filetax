...
const fmtDate = (iso: string | null | undefined): string => {
  ...
  if (!y || !m || !d || parts.length !== 3) {
    // Malformed ISO string — log and return empty rather than writing garbage into a PDF field
...
const fill5472 = async (
  ...
  // comparing with the incorporation year derived from date_of_incorporation.
...
const fill1120 = async (
  ...
  setText(doc, F.DATE_INCORPORATED, filing.date_of_incorporation ? fmtDate(filing.date_of_incorporation) : '');
