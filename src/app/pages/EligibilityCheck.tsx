...
import { PRICE_PER_YEAR, PRICE_RCL, PRICE_ADDITIONAL_PARTY, additionalPartiesCost, additionalPartiesBreakdown } from "../../lib/pricing";

// ---------------------------------------------------------------------
// Service go-live flag.
//   WAITLIST_MODE = true  → eligibility CTA routes to /waitlist (pre-launch).
//   WAITLIST_MODE = false → eligibility CTA routes to /portal, carrying the
//                           eligibility answers as query params so the filing
//                           is seeded (see buildPortalPath + Portal.tsx +
//                           Dashboard.startFromEligibility).
// Flip this single flag to switch the funnel live.
// ---------------------------------------------------------------------
const WAITLIST_MODE = false;
const PORTAL_PATH = WAITLIST_MODE ? "/waitlist" : "/portal";
      ...
      <section style={{ background: "var(--tf-bg)", minHeight: "80vh", padding: "3rem 1rem" }}>
        <div style={{ maxWidth: "580px", margin: "0 auto" }}>
          <div style={{ background: "var(--tf-surface)", border: "1px solid var(--tf-border)", borderRadius: "0.75rem", padding: "2rem", boxShadow: "0 1px 2px oklch(0.2 0.01 80 / 0.06), 0 4px 16px oklch(0.2 0.01 80 / 0.04)" }}>
            ...
            <div style={{ background: "var(--tf-bg)", border: "1px solid var(--tf-border)", borderRadius: "0.625rem", padding: "1.25rem", marginBottom: "1.25rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem", marginBottom: "0.875rem" }}>
                ...
                {answers.includeRCL && hasPriorYears && (
                  <PriceRow
                    label="CPA-Prepared Reasonable Cause Letter (one letter, covers all years)"
                    value={`+$${PRICE_RCL}`}
                  />
                )}
                ...
                {additionalParties > 0 && (
                  <PriceRow
                    label={`Additional related part${additionalParties > 1 ? "ies" : "y"} (${additionalParties} more${numYears > 1 ? `, ${numYears} years` : ""})`}
            ...
            >
              {WAITLIST_MODE ? "Join the Waitlist" : "Create Your Free Account to Begin"}
            </Link>
            <p style={{ color: "var(--tf-muted)", fontSize: "0.8125rem", fontWeight: 400, textAlign: "center", lineHeight: 1.6 }}>
              {WAITLIST_MODE
                ...
                : "Your answers carry over automatically. No payment until you download your forms."}
            </p>
          </div>
        </div>
      </section>
            ...
            <div>
              ...
              {showPriorYearNote ? (
                  ...
                  <div style={{ border: "2px solid #0284C7", borderRadius: "0.75rem", padding: "1.25rem 1.375rem", marginBottom: "1rem", background: "rgba(2,132,199,0.03)" }}>
                    <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: "1rem", marginBottom: "0.875rem" }}>
                      ...
                      <div style={{ textAlign: "right", flexShrink: 0 }}>
                        <p style={{ fontWeight: 700, fontSize: "1.125rem", color: "var(--tf-text)", lineHeight: 1 }}>+${PRICE_RCL}</p>
                        <p style={{ fontSize: "0.8125rem", color: "var(--tf-muted)", fontWeight: 400 }}>one letter, all years</p>
                      </div>
                    </div>
                ...
                <div>
                  ...
                  <div style={{ border: "2px solid #0284C7", borderRadius: "0.75rem", padding: "1.25rem 1.375rem", marginBottom: "1rem", background: "rgba(2,132,199,0.03)" }}>
                    ...
                    <p style={{ fontWeight: 700, fontSize: "1rem", color: "var(--tf-text)", marginBottom: "0.625rem" }}>
                      One CPA-Prepared Reasonable Cause Letter covers every year
                    </p>
                    <p style={{ fontSize: "0.875rem", color: "var(--tf-text)", lineHeight: 1.65, marginBottom: "0.875rem" }}>
                      Filing three or more years at once is handled through our multi-year package. A single Reasonable Cause Letter covers all the years you file. Pricing is confirmed at checkout: ${PRICE_PER_YEAR} per year plus one ${PRICE_RCL} letter.
                    </p>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <OptionButton
                    label="2 prior years"
                    sublabel="One Reasonable Cause Letter covers both years."
